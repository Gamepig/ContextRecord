...
async def enable_auto_recording():
    ...
    enable_result = await handle_call_tool(
        ...
        {"session_id": "default", "record_user": True, "record_assistant": True},
    )
    ...
    if enable_data["success"]:
        ...
        print(
            f'配置: 啟用={config.get("enabled")}, 用戶訊息={config.get("record_user")}, 助理回應={config.get("record_assistant")}'
        )
    ...
    status_result = await handle_call_tool(
        "get_auto_recording_status", {"session_id": "default"}
    )
    ...
    print(
        f'詳細配置: 啟用={config.get("enabled")}, 用戶={config.get("record_user")}, 助理={config.get("record_assistant")}'
    )
