...
async def call_tool(tool_call: ToolCall):
    ...
    try:
        ...
        # 返回工具執行結果
        return ToolResponse(request_id=tool_call.request_id, result=result)
    except Exception as e:
        # 如果執行過程中發生錯誤，返回錯誤信息
        return ToolResponse(request_id=tool_call.request_id, result=None, error=str(e))
...
async def list_tools():
    ...
    for name, tool_info in tool_registry.items():
        tools_list.append({"name": name, "description": tool_info["description"]})
...
def register_conversation_tools():
    ...
    try:
        ...
        # Warning: Could not register conversation tools - removed print statement
        pass
